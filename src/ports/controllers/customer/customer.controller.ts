...
export class CustomerController {
    ...
    async getCustomerById(@Param('id') id: Types.ObjectId, @Res() response: any) {
        try {
            const customer = await this.customerService.findUniqueOrFail(
                { _id: id }
            )
    ...
    @Put('/:id')
    async updateCustomer(@Param('id') id: string, @Body() body: any, @Res() response: any) {
        try {
            const updatedCustomer = await this.customerService.updateCustomer(id, body);
