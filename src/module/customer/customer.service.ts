...
export class CustomerService {
    constructor(
        @Inject("customer.store") private readonly customers: CustomerStore
    ) { }
    ...
    async updateCustomer(id: string, partial: Partial<ICustomer>): Promise<ICustomer> {
    ...
    async findCustomer(filters: Partial<ICustomer>): Promise<ICustomer> {
        const customer = await this.customers.getOneByAtLeastOneCondition([filters])
